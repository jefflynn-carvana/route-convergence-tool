...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .share-info h3 { color: #1976d2; margin-bottom: 10px; }
        .share-url {
            background: white; border: 1px solid #ddd; border-radius: 4px; padding: 10px;
            font-family: monospace; word-break: break-all; margin: 10px 0; cursor: pointer;
        }
        .routes-container { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; margin-bottom: 40px; }
        .route-section {
            background: #f8f9fa; border-radius: 12px; padding: 30px; border: 2px solid #e9ecef;
            transition: all 0.3s ease;
        }
        .route-section:hover { border-color: #2596be; box-shadow: 0 8px 25px rgba(37, 150, 190, 0.15); }
        .route-section h2 {
            color: #2596be; margin-bottom: 25px; font-size: 1.5em;
            border-bottom: 2px solid #2596be; padding-bottom: 10px;
        }
        .form-group { margin-bottom: 20px; }
        label { display: block; margin-bottom: 8px; font-weight: 600; color: #495057; font-size: 0.95em; }
        input, select {
            width: 100%; padding: 12px 15px; border: 2px solid #dee2e6; border-radius: 8px;
            font-size: 16px; transition: all 0.3s ease; background: white;
        }
        input:focus, select:focus {
            outline: none; border-color: #2596be; box-shadow: 0 0 0 3px rgba(37, 150, 190, 0.1);
        }
        .controls { text-align: center; margin-bottom: 40px; }
        .btn {
            background: linear-gradient(135deg, #2596be 0%, #1e7ba8 100%); color: white; border: none;
            padding: 15px 40px; font-size: 1.1em; border-radius: 8px; cursor: pointer;
            transition: all 0.3s ease; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;
        }
        .btn:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(37, 150, 190, 0.3); }
        .result {
            margin-top: 30px; padding: 25px; border-radius: 12px; text-align: center;
            font-size: 1.2em; font-weight: 600; min-height: 80px; display: flex;
            align-items: center; justify-content: center; transition: all 0.5s ease;
            opacity: 0; transform: translateY(20px);
        }
        .result.show { opacity: 1; transform: translateY(0); }
        .result.success { background: linear-gradient(135deg, #28a745 0%, #20c997 100%); color: white; border: 3px solid #1e7e34; }
        .result.error { background: linear-gradient(135deg, #dc3545 0%, #e74c3c 100%); color: white; border: 3px solid #c82333; }
        .details {
            margin-top: 20px; padding: 20px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #2596be;
        }
        .details h3 { color: #2596be; margin-bottom: 15px; }
        .details p { margin-bottom: 8px; line-height: 1.6; }
        .footer { text-align: center; padding: 20px; background: #f8f9fa; border-top: 1px solid #dee2e6; color: #6c757d; }
        .copy-btn {
        ...
        .spinner {
            border: 4px solid #f3f3f3; border-top: 4px solid #2596be; border-radius: 50%;
            width: 40px; height: 40px; animation: spin 1s linear infinite; margin: 0 auto 10px;
        }
        @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
        @media (max-width: 768px) {
            .routes-container { grid-template-columns: 1fr; gap: 20px; }
    ...
    <div class="container">
        ...
        <div class="content">
            <div class="share-info">
                ...
                <div class="share-url" id="shareUrl" onclick="copyToClipboard()">Loading URL...</div>
            ...
            <div class="routes-container">
                <div class="route-section">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="location1" placeholder="e.g., NYC, 10001, New York">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="destLocation1">Destination Location Code:</label>
                        <input type="text" id="destLocation1" placeholder="e.g., LAX, 90210, Los Angeles">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="destination1">Destination Address:</label>
                        <input type="text" id="destination1" placeholder="Ending address">
                    </div>
                    <div class="form-group">
                ...
                <div class="route-section">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="location2" placeholder="e.g., CHI, 60601, Chicago">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="destLocation2">Destination Location Code:</label>
                        <input type="text" id="destLocation2" placeholder="e.g., MIA, 33101, Miami">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="destination2">Destination Address:</label>
                        <input type="text" id="destination2" placeholder="Ending address">
                    </div>
                    <div class="form-group">
    ...
    <script>
        class RouteConvergenceTool {
            ...
            getRouteData(routeNumber) {
                ...
                const destLocation = document.getElementById(`destLocation${routeNumber}`).value.trim();
                ...
                if (!location || !address || !destLocation || !meetpointCode || !destination || !duration || !startTime) {
                    throw new Error(`Please fill in all fields for Route ${routeNumber}`);
                }
                return {
                    location, address, destLocation, meetpointCode, destination, startTime, duration,
                    originTimezone: this.detectTimezone(location), destinationTimezone: this.detectTimezone(destLocation)
                };
            }
            ...
            displayResult(analysis) {
                ...
                const details = `
                    <div class="details">
                        ...
                        <p><strong>Destination Similarity:</strong> ${(analysis.destinationSimilarity * 100).toFixed(1)}%</p>
